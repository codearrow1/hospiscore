...
const LOCKED_PLACEHOLDER = [82, 64, 45, 71];
...
export default function ScoreCheckWidget() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/report", {
        ...
        body: JSON.stringify({ name, email, propertySlug: selected.id }),
      });
      ...
      if (detail.ok) {
        const payload = (await detail.json()) as { score: ScoreData };
        setUnlocked(payload.score);
      } else {
        setUnlocked({
      ...
      setTimeout(() => unlockRef.current?.scrollIntoView({ behavior: "smooth", block: "nearest" }), 80);
    } catch {
      setLead({ status: "error", message: "Could not reach the server. Try again." });
    }
  }

  const bars = unlocked && unlocked.components.length > 0
  ...
  return (
    <div className="w-full">
            ...
            <li key={r.id}>
              ...
              >
                ...
                <div className="flex shrink-0 items-center gap-3">
                  ...
                  <span className={`flex h-10 w-10 items-center justify-center rounded-xl text-sm font-bold tabular-nums ${gradeTone(r.overall)}`}>
                    {r.overall}
                  </span>
                </div>
              </button>
      ...
      {selected && (
        <div className="mt-4 rounded-3xl border border-zinc-800 bg-zinc-900/70 p-5 sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <ScoreGauge score={selected.overall} size={96} />
            </div>
          </div>

          <div ref={unlockRef} className="mt-5">
            {unlocked && bars ? (
              <div className="space-y-4">
                <div className="flex flex-wrap gap-2">
                  {unlocked.components
                    ...
                    .slice(0, 2)
                    .map((c) => (
                      <span key={c.key} className="rounded-full bg-emerald-500/15 px-3 py-1 text-xs font-medium text-emerald-400">
                        ✓ {c.label}
                      </span>
                    ))}
                  {unlocked.components
                    ...
                    .slice(0, 2)
                    .map((c) => (
                      <span key={c.key} className="rounded-full bg-rose-500/15 px-3 py-1 text-xs font-medium text-rose-400">
                        ! {c.label}
                      </span>
                    ))}
                </div>
                ...
                <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-xs text-zinc-500">
                    {unlocked.platformsCount} platforms · {unlocked.dataCompleteness}% data coverage ·{" "}
                    {unlocked.totalReviews.toLocaleString("en-US")} reviews
                  </p>
                  <Link
                    href={selected.isLive ? `/property/${selected.id}` : `/properties/${selected.slug}`}
                  ...
                  >
                    Open full report
                    <svg className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                      <path fillRule="evenodd" d="M3 10a.75.75 0 0 1 .75-.75h10.6L10.6 5.4a.75.75 0 1 1 1.06-1.06l4.97 4.97a.75.75 0 0 1 0 1.06l-4.97 4.97a.75.75 0 0 1-1.06-1.06l3.75-3.75H3.75A.75.75 0 0 1 3 10Z" clipRule="evenodd" />
                    </svg>
                  </Link>
                </div>
              </div>
              ...
              <div className="relative overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950/60">
                <div className="pointer-events-none select-none p-5" aria-hidden="true">
                  ...
                  <div className="mb-4 h-2.5 w-40 rounded bg-zinc-800" />
                  <div className="h-2.5 w-56 rounded bg-zinc-800" />
                </div>
                <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-b from-zinc-950/30 via-zinc-950/70 to-zinc-950/95">
                ...
                <form onSubmit={onSubmit} className="relative z-10 border-t border-zinc-800 p-5">
                  <div className="grid gap-3 sm:grid-cols-2">
                    <input
                      ...
                      className="w-full rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-2.5 text-sm text-zinc-50 outline-none transition placeholder:text-zinc-500 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-900"
                    />
                    <input
                      type="email"
                      required
                  ...
                  >
                    {lead.status === "loading" ? "Unlocking…" : "Unlock the full report — it’s free"}
                  </button>
                  {lead.status === "error" && (
                    <p role="alert" className="mt-3 rounded-xl border border-red-900 bg-red-950/40 p-3 text-sm text-red-300">
                      {lead.message}
                    </p>
                  )}
                  {lead.status === "success" && (
                    <p role="status" className="mt-3 rounded-xl border border-emerald-900 bg-emerald-950/40 p-3 text-sm text-emerald-300">

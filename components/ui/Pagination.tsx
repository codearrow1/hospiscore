...
export function Pagination({
  ...
  const linkCls =
    "inline-flex min-h-8 items-center rounded-lg border border-line px-2.5 py-1 text-xs font-semibold transition hover:bg-surface-subtle disabled:pointer-events-none disabled:opacity-40";
          ...
          <>
            ...
            {pages.map((p) => (
              <button
                ...
                className={`${linkCls} ${p === page ? "border-indigo-300 bg-brand-soft text-brand dark:border-indigo-700" : ""}`}
              >
                {p}
              </button>
            ))}
...
export function FilterChip({
  ...
  const cls = `inline-flex min-h-7 items-center rounded-full border px-2.5 py-0.5 text-xs font-medium transition ${
    active

            ...
            <div className={`grid gap-4 ${detailed ? "md:grid-cols-2 xl:grid-cols-3" : "sm:grid-cols-2 lg:grid-cols-3"}`}>
              {modules.map((m) => (
                <div
                  key={m.id}
                  id={m.id}
                  className="glow-border group scroll-mt-24 rounded-2xl border border-zinc-800 bg-zinc-900 p-5 transition hover:-translate-y-0.5 hover:border-zinc-600 hover:shadow-md"
                >
                  <Link href={`/platform#${m.id}`}>
                    <h4 className="flex items-center gap-2 font-semibold text-zinc-100">
                      <Icon name={cat.icon} className="h-4 w-4 text-indigo-400 transition duration-300 group-hover:scale-125 group-hover:text-indigo-300" />
                      {m.name}
                    </h4>
                  ...
                  {detailed && (
                    <ul className="mt-3 flex flex-col gap-1.5 border-t border-zinc-800 pt-3">
                      {m.bullets.map((b) => (

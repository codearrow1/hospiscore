...
export async function createSubscription(input: {
  ...
  const [plan, org] = await Promise.all([
    ...
    prisma.organization.findUnique({ where: { id: input.organizationId }, select: { id: true, affiliateId: true } }),
  ]);
  ...
  // Auto-create commission if org was acquired via affiliate
  if (org.affiliateId) {
    try {
...
export async function renewSubscription(id: string) {
  ...
  return prisma.subscription.update({ where: { id }, data: { currentPeriodStart: sub.currentPeriodEnd, currentPeriodEnd: nextEnd, status: "active" } });
}

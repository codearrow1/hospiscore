...
export default function MarketingAdminError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[marketing-admin] route error", error);
  }, [error]);
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-2xl border border-line bg-surface px-6 py-16 text-center">
      ...
      <p className="mt-1 max-w-md text-xs text-zinc-500 dark:text-zinc-400">
        The growth workspace hit an unexpected error. Retry — if it persists contact support with
        the reference below.
      </p>
      {error.digest && (

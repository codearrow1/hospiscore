...
const SAAS_NAV: (NavItem & { perm?: SaasPermission })[] = [
  ...
  { href: "/saas/financial-approvals", label: "Financial Approvals", perm: "FINANCIAL_APPROVE" },
...
{ href: "/saas/settings/payments", label: "Payments", perm: "SYSTEM_SETTINGS_MANAGE" },
...
export default async function SaasLayout({ children }: { children: ReactNode }) {
  ...
  await initSaasDb().catch((e) => console.error("[saas-layout] db init failed:", e?.message ?? e));
  if (!canAccess(user)) {

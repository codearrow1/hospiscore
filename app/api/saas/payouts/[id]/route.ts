...
async function payoutAmount(id: string): Promise<number> {
  const row = await prisma.affiliatePayout.findUnique({ where: { id }, select: { amount: true } });
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    // Server-side four-eyes gate: releasing a payout to `paid` is the highest
    // risk financial step and cannot bypass approval when the policy requires it.
    if (status === "paid" && (await requiresApproval("payout.release", await payoutAmount(id)))) {
      return NextResponse.json(
        { error: "Four-eyes approval required to release this payout. Submit a financial approval request first.", action: "financial_approval" },
        { status: 409 },
      );
    }
